...
import { millisecondsToSeconds, downloadAudioClip, fetchClip, cleanFilename } from '~/aa-util';
...
const props = defineProps<{
  ...
  const trackPlayhead = async () => {
    ...
    if (!audioObj.value) {
      console.error('Track playhead: Audio object for ' + props.audioId + ' is null');
      return;
    }
    const interval = setInterval(() => {
      ...
      if (audioObj.value.currentTime >= clipDuration.value - .01) {
        clearInterval(interval);
        props.onSetPlaying(null);
        return;
      }
  ...
  watch(() => props.playingClip, (playing) => {
    if (playing) {
...
<template>
  <div class="flex h-[300px] w-full flex-row items-center gap-4 border px-6 py-4">
